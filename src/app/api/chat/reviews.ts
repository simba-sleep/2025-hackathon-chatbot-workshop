// Reads and parses SentimentTest1.csv for all reviews
export function getAllProductReviews(): (Review & { productId?: string })[] {
  ...
  const reviews: (Review & { productId?: string })[] = [];
  for (let i = 1; i < lines.length && reviews.length < 500; i++) {
    const row = lines[i];
    if (!row.trim()) continue;
    const cols = row.split(/,(?=(?:[^"]*"[^"]*")*[^"]*$)/); // handle quoted commas
    reviews.push({
      productId: cols[productIdIdx]?.trim() || undefined,
...
export function getProductReviews(productId: string): Review[] {
  ...
  for (let i = 1; i < lines.length && reviews.length < 1000; i++) {
    const row = lines[i];
    if (!row.trim()) continue;
    const cols = row.split(/,(?=(?:[^"]*"[^"]*")*[^"]*$)/); // handle quoted commas
